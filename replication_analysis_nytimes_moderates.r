...
wd_models_comp$comp <- ifelse(wd_models_base$dem_total_pac_contribs <= 1000,'No PAC funding',
                                ifelse(wd_models_base$dem_pac_mod_nyt =='mod','Moderate',
                                ifelse(wd_models_base$dem_pac_prog_nyt =='prog','Progressive','Non-Moderate')))                                
...
get_3way_plot <- function(summary_3way) {
  ...
  plot_3way <- ggplot(summary_3way, aes(x = avg_overperformance, y = fct_rev(Category), fill = avg_overperformance > 0)) +
    ...
    # --- Add the bracket and text annotation ---
    ...
    labs(
      title = "The NYT's 'Non-Moderate' Group Is a Biased Comparison",
      subtitle = "It conflates serious, funded 'Non-Moderates' with non-competitive 'No PAC Funding' candidates, skewing the results.",
...
# ==============================================================================
# SECTION 5: PLOT 2 (DISAPPEARING EFFECT - 2 BARS)
# ==============================================================================

# Filter for Democratic candidates with *significant* PAC contributions (>= 1000)
wd_plot2_filtered <- wdd_2024 %>%
  mutate(
    dem_pac_mod_nyt_numeric = as.numeric(dem_pac_mod_nyt == 'mod')
    # Note: rep_pac_mod_nyt not needed for this plot
  ) %>%
  filter(dem_total_pac_contribs >= 1000)

# 1. Unadjusted (Simple) Model
dem.model.simple.p2 <- feols(dem_diff ~ dem_pac_mod_nyt_numeric, data = wd_plot2_filtered)

# 2. Adjusted (Controlled) Model
dem.model.controlled.p2 <- feols(dem_diff ~ dem_pac_mod_nyt_numeric + dem.fundraising + rep.fundraising + incumbency_status, data = wd_plot2_filtered)

# --- Extract Coefficients for Plotting ---
# Tidy the models to get coefficients and p-values
tidy_simple_p2 <- tidy(dem.model.simple.p2) %>% filter(term == "dem_pac_mod_nyt_numeric")
tidy_controlled_p2 <- tidy(dem.model.controlled.p2) %>% filter(term == "dem_pac_mod_nyt_numeric")

# Create a single data frame for plotting
plot_data_p2 <- rbind(tidy_simple_p2, tidy_controlled_p2)
plot_data_p2$Model <- c("Unadjusted", "Adjusted")

# --- Create the "Sideways" Bar Chart ---
disappearing_effect_plot_2 <- plot_data_p2 %>%
  mutate(
    # Convert share (e.g., 0.0088) to percentage points (e.g., 0.88)
    estimate_pp = estimate * 100,
    # Create labels, adding a star if p.value < 0.05
    label_text = sprintf("%+.2f pts.%s",
                         estimate_pp,
                         ifelse(p.value < 0.05, "*", ""))
  ) %>%
  ggplot(aes(x = estimate_pp, y = fct_rev(Model), fill = Model)) +
  geom_col() +
  
  # Add the text labels to the end of the bars
  geom_text(aes(label = label_text),
            hjust = 0,
            nudge_x = 0.05, # Place text just to the right
            size = 5,
            fontface = "bold") +
  ...
  scale_fill_manual(values = c("Unadjusted" = "#A066D3", "Adjusted" = "#A066D3"), guide = "none") +
  
  labs(
    title = "The Disappearing Effect of Moderation",
    subtitle = "The vote share advantage for moderates shrinks to near zero after controlling for basic factors.",
...
# --- Save the Plot ---
ggsave("disappearing_effect_horizontal_chart.png", disappearing_effect_plot_2, width = 10, height = 5, dpi = 300)

print("Plot 2 (2-bar disappearing effect) saved:")
print("- disappearing_effect_horizontal_chart.png")


# ==============================================================================
# SECTION 6: PLOT 3 (DISAPPEARING EFFECT - 3 BARS, WITH NYT CLAIM)
# ==============================================================================
...
wd_plot3_filtered <- wdd_2024 %>%
  ...
  filter(dem_total_pac_contribs >= 10000)
...
# --- Save the Plot ---
ggsave("disappearing_effect_bar_chart.png", disappearing_effect_plot_3, width = 10, height = 5, dpi = 300)
